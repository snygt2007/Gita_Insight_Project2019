...
dir_litw_super_cleaned=Path("./data/filtered_supervised_image_v9")
...
def resize_images_df(folder_path_global, company_name,dir_litw_resized, expected_dimension,size_threshold, folder_logo_num):
    df_logos_pickle = pd.DataFrame(columns=['true_logo','logo_img_name','original_image_name','company_brand','folder_path','mean_mean_val','mean_std_val','mscn_val'])
...
def get_folder_stats(df_logos_folder_label, df_folder_details,dir_path):
    ...
    df_logos_folder = pd.DataFrame(columns=['company_brand','folder_mean_val','folder_std_val','folder_mscn_val','folder_path']) 
    ...
    for company_name in folders:
        ...
        row = pd.Series({'company_brand' :company_name,
                            ...
                            'folder_path': df_rows["folder_path"]})
...
tp_df_image_resized.to_csv('folder_image_Details_7022019_v3.csv')
df_logos_folder_label.to_csv('folder_Stats_7022019_v3.csv')
...
tp_df_excluded=test_data_exclusion_1(df_logos_folder,A,B,C, dir_litw_super_cleaned,dir_litw_resized)


# In[37]:


Print_filtered_folder_details(dir_litw_super_cleaned)
